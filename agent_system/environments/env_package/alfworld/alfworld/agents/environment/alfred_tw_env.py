...
class AlfredTWEnv(object):
    ...
    def init_env(self, batch_size):
        ...
        # Multiply by 2 to account for the extra inventory step called after each
        # real action in the training loop (rollout_loop.py). Without this,
        # TextWorld's internal step counter fires at max_nb_steps_per_episode/2
        # real steps instead of the intended max_nb_steps_per_episode.
        env_id = textworld.gym.register_games(self.game_files, request_infos,
                                              batch_size=batch_size,
                                              asynchronous=True,
                                              max_episode_steps=max_nb_steps_per_episode * 2,
                                              wrappers=wrappers)
