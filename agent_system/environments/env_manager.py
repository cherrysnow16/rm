...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def get_inventory(self, batch_size: int) -> List[str]:
        """Query inventory state without polluting self.memory or self.pre_text_obs.

        Calls self.envs.step() directly, bypassing AlfWorldEnvironmentManager.step(),
        so the history buffer and pre_text_obs are unaffected.
        Uses the same Ray parallel fan-out as step() — no additional bottleneck.
        """
        text_obs, _, _, _, _ = self.envs.step(["inventory"] * batch_size)
        return text_obs
    ...
    def state_preprocess(self, total_batch_list, tokenizer):
        ...
        for i, traj in enumerate(total_batch_list):
            ...
            for j in range(1, len(traj)):
                ...
                # Build obj_state_map from the accumulated object_state_dict so that
                # cooled/heated/cleaned annotations persist across all subsequent steps,
                # not only the step where the event was first detected.
                obj_state_map = {}
...
class SciWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        ...
        # Extract task descriptions from infos
    ...
    def build_text_obs(
        ...
        """Build text observations with history."""
        postprocess_text_obs = []
        ...
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            ...
            if batch_item['active_masks']:
                ...
                if task_num is not None:
                    success[f'task_{task_num}_success_rate'].append(won_value)
                return
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        variations_idx_path = os.path.join(
            ...
            'env_package/sciworld/variations_idx/L0_idx.json'
        )
        import json
        ...
        env_kwargs = {
            'jar_path': None,
            'env_step_limit': config.env.max_steps,
