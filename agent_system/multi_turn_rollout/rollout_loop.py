...
class TrajectoryCollector:
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            # Get inventory BEFORE the action so inventory_state is consistent with
            # anchor_obs (both reflect the state at the moment of deciding action j).
            # For already-done envs the data has active_masks=False and state_preprocess
            # overwrites it with "You Won!", so stale inventory for those envs is harmless.
            if self.config.env.env_name == "alfworld/AlfredTWEnv":
                batch.non_tensor_batch["inventory_state"] = envs.get_inventory(batch_size)
    ...
    def multi_turn_loop(
        ...
        elif self.config.algorithm.adv_estimator == "tpab_rewardflow" and is_train:
            from tpab_rewardflow import apply_tpab_rewardflow_propagation
            total_batch_list = apply_tpab_rewardflow_propagation(
                total_batch_list, self.config, self.tokenizer, envs, actor_rollout_wg
            )
